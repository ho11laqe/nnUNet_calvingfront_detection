    ...
    def evaluate(self, test=None, reference=None, advanced=False, **metric_kwargs):
        ...
        if isinstance(self.labels, dict):
            ...
            for j, label in enumerate(self.labels):
                self.result[j] = OrderedDict()
                for i, l in enumerate(label):
                    # TODO add second label (Done)
                    k = str(l)
                    self.result[j][k] = OrderedDict()
                    self.confusion_matrix.set_test(self.test[j] == l)
                    self.confusion_matrix.set_reference(self.reference[j] == l)
                    for metric in eval_metrics:
                        self.result[j][k][metric] = _funcs[metric](confusion_matrix=self.confusion_matrix,
                                                                   nan_for_nonexisting=self.nan_for_nonexisting,
                                                                   **metric_kwargs)
...
class NiftiEvaluator(Evaluator):
    ...
    def set_test(self, test):
        ...
        if test is not None:
            # TODO test images has only zone prediction Look at the code where image is saved. (Done)
...
def aggregate_scores(test_ref_pairs,
    ...
    # all_res= [run_evaluation((test[0], ref[0], evaluator, metric_kwargs))]
    ...
    for mask in range(len(labels)):
        all_scores[mask] = OrderedDict()
        all_scores[mask]["all"] = []
        all_scores[mask]["mean"] = OrderedDict()
        for i in range(len(all_res)):
            all_scores[mask]["all"].append(all_res[i][mask])

            # append score list for mean
            for label, score_dict in all_res[i][mask].items():
                if label in ("test", "reference"):
                    continue
                if label not in all_scores[mask]["mean"]:
                    all_scores[mask]["mean"][label] = OrderedDict()
                for score, value in score_dict.items():
                    if score not in all_scores[mask]["mean"][label]:
                        all_scores[mask]["mean"][label][score] = []
                    all_scores[mask]["mean"][label][score].append(value)

        for label in all_scores[mask]["mean"]:
            for score in all_scores[mask]["mean"][label]:
                if nanmean:
                    all_scores[mask]["mean"][label][score] = float(np.nanmean(all_scores[mask]["mean"][label][score]))
                else:
                    all_scores[mask]["mean"][label][score] = float(np.mean(all_scores[mask]["mean"][label][score]))
