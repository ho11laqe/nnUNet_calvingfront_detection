...
if __name__ == '__main__':
    ...
    task_name = 'Task501_Glacier_front'
    ...
    # labels
    labels_dir_tr = join(base, 'fronts_dilated_5', 'train')
    ...
    for label_tr in training_cases_sampled:
        unique_name = label_tr[:-len('_front.png')]  # just the filename with the extension cropped away, so img-2.png becomes img-2 as unique_name
        print(unique_name)
    ...
    labels_dir_ts = join(base, 'fronts_dilated_5', 'test')
    ...
    for label_ts in testing_cases_sampled:
        unique_name = label_ts[:-len('_front.png')]
        print(unique_name)
    ...
    generate_dataset_json(join(target_base, 'dataset.json'), target_imagesTr, target_imagesTs, ('SAR',),
                          labels={'label0':{0: 'background', 1: 'front'}}, dataset_name=task_name, license='hands off!')
