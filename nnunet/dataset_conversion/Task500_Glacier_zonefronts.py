...
if __name__ == '__main__':
    ...
    # labels
    labels_dir_tr = join(base, 'zone_fronts', 'train')
    ...
    for label_tr in training_cases_sampled:
        unique_name = label_tr[:-len('_.png')]  # just the filename with the extension cropped away, so img-2.png becomes img-2 as unique_name
    ...
    labels_dir_ts = join(base, 'zone_fronts', 'test')
    ...
    for label_ts in testing_cases_sampled:
        unique_name = label_ts[:-len('_.png')]
        print(unique_name)
