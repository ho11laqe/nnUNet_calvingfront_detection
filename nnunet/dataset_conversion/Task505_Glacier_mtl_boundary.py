...
if __name__ == '__main__':
    ...
    task_name = 'Task505_Glacier_mtl_boundary'
    ...
    # labels
    label_fronts_dir_tr = join(base, 'fronts_dilated_5', 'train')
    label_zones_dir_tr = join(base, 'zones', 'train')
    label_boundary_dir_tr = join(base, 'boundaries_dilated_5', 'train')
    ...
    for label_front_tr in training_cases_sampled:
        ...
        label_front_tr = unique_name + '_front.png'
        label_boundary = unique_name + '_boundary.png'

        input_front_file = join(label_fronts_dir_tr, label_front_tr)
        input_zone_file = join(label_zones_dir_tr, label_zone_tr)
        input_boundary_file = join(label_boundary_dir_tr, label_boundary)
        ...
        convert_mtl_boundary_image_to_nifti(input_front_file, input_zone_file, input_boundary_file, output_seg_file, is_seg=True)

    # now do the same for the test set
    label_fronts_dir_ts = join(base, 'fronts', 'test')
    label_zones_dir_ts = join(base, 'zones', 'test')
    label_boundary_dir_ts = join(base, 'boundaries', 'test')
    ...
    generate_dataset_json(join(target_base, 'dataset.json'), target_imagesTr, target_imagesTs, ('SAR',),
                          labels={'label0': {0: 'background', 1: 'front'},
                                  ...
                                  'label2': {0: 'background', 1: 'boundary'}},
