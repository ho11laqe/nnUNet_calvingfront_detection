...
class NetworkTrainer(object):
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        #
        #new_state_dict['tu_1.0.weight'] = new_state_dict['tu_1.0.weight'].transpose(2, 3)
        #new_state_dict['tu_2.0.weight'] = new_state_dict['tu_2.0.weight'].transpose(2, 3)
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            # Create GIF from training
            #self.store_sample_prediction()

            print(psutil.virtual_memory())
            self.print_to_log_file(psutil.virtual_memory())
    ...
    def store_sample_prediction(self):
        self.network.eval()
        maybe_mkdir_p(self.output_folder+'/gif/')
        output_filename = self.output_folder+'/gif/Crane_2008-11-04_TSX_7_1_034_'+str(self.epoch) +'_.nii.gz'
        file_path = self.dataset_directory + '/nnUNetData_plans_mtl_2D_stage0/Crane_2008-11-04_TSX_7_1_034.npy'
        properties_path = self.dataset_directory+ '/nnUNetData_plans_mtl_2D_stage0/Crane_2008-11-04_TSX_7_1_034.pkl'
        with open(properties_path, 'rb') as f:
            properties = pickle.load(f)
        img = np.load(file_path)[0][None]
        softmax = self.predict_preprocessed_data_return_seg_and_softmax(
            img, do_mirroring=True, mirror_axes=self.data_aug_params['mirror_axes'], use_sliding_window=True,
            step_size=0.5, use_gaussian=True, all_in_gpu=False,
            mixed_precision=True)[1]

        save_segmentation_nifti_from_softmax(softmax, output_filename, properties)
        return
